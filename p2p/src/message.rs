...
use crate::{message_blockdata, Magic};

#[rustfmt::skip]                // Keep public re-exports separate.
#[doc(no_inline)]
pub use self::error::{
    AddrPayloadDecoderError, AddrV2PayloadDecoderError, CommandStringDecoderError, FeeFilterDecoderError,
    CommandStringError, HeadersMessageDecoderError, InventoryPayloadDecoderError,
    NetworkHeaderDecoderError, PingDecoderError, PongDecoderError, V1MessageHeaderDecoderError 
};
...
/// Serializer for messages that have no body (are empty).
#[derive(Clone, PartialEq, Eq, Debug)]
pub struct Empty;
...
impl encoding::Encoder for EmptyEncoder {
    fn current_chunk(&self) -> &[u8] { &[] }

    fn advance(&mut self) -> bool { false }
}

impl encoding::ExactSizeEncoder for EmptyEncoder {
    #[inline]
    fn len(&self) -> usize { 0 }
}

impl encoding::Encode for Empty {
    type Encoder<'e>
        = EmptyEncoder
    where
        Self: 'e;

    fn encoder(&self) -> Self::Encoder<'_> { EmptyEncoder }
}
